...
export const description = '投篮大赛 - 比拼投篮得分';
...
function handleMessage(msg: GameMessage, sid: string, ws: WebSocket): void {
  if (msg.type === 'match:join' || msg.type === 'room:create') {
    ...
    setTimeout(() => {
      ...
      m.filter((x) => !x.isBot).forEach((x) => {
        if (x.ws) {
          send(x.ws, 'match:found', { roomId: id, position: m.indexOf(x), seed: room.seed });
          socketToRoom.set(x.id, id);
        }
      });
      broadcast(room, 'game:start', {
        seed: room.seed,
        players: room.players.map((p) => ({ id: p.id, nick: p.nick, side: p.side })),
      });
    }, 8000);
  }
  ...
  if (msg.type === 'game:state') {
    for (const [, room] of rooms) {
      const p = room.players.find((player) => player.id === sid);
      if (p) {
        broadcast(room, 'game:state', { id: sid, state: msg.data });
        break;
      }
    }
  }
...
router.get('/health', (_req, res) => res.json({ status: 'ok', game: 'basketball', rooms: rooms.size }));
...
export function socketSetup(httpServer: HttpServer, wsPath: string): void {
  ...
  wsServer.on('upgrade', (req, s, head) => {
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (raw: RawData) => {
      try {
        ...
        console.error('[basketball] 消息处理错误:', err);
      }
    });
    ws.on('close', () => {
      clients.delete(sid);
