...
export const description = '太空射击 - 多人弹幕';

export const router = Router();

interface SpaceShooterPlayer extends Player {
  side?: number;
  alive?: boolean;
}
  ...
  if (msg.type === 'match:join' || msg.type === 'room:create') {
    ...
    setTimeout(() => {
      ...
      broadcast(room, 'game:start', {
        seed: room.seed,
        players: room.players.map((p) => ({ id: p.id, nick: p.nick, side: (p as SpaceShooterPlayer).side })),
      });
    }, 8000);
  }
    ...
    for (const [, room] of rooms) {
      ...
      if (p) {
        ...
        if (action.alive === false) {
          p.alive = false;
          const alive = room.players.filter((pl) => (pl as SpaceShooterPlayer).alive);
          if (alive.length <= 1) {
            room.phase = 'finished';
            broadcast(room, 'game:over', {
              ...
              scores: room.players.map((pl) => ({
                nick: pl.nick,
                score: (pl as SpaceShooterPlayer).score,
              })),
            });
          }
        }
...
router.get('/health', (_req, res) => res.json({ status: 'ok', game: 'space-shooter' }));
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (raw: RawData) => {
      try {
        ...
        console.error('[space-shooter] 消息处理错误:', err);
      }
    });
    ws.on('close', () => {
      clients.delete(sid);
