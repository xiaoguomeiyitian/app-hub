...
export const description = '飞镖 - 轮流投掷比高分';

export const router = Router();

interface DartsPlayer extends Player {
  side: number;
  score: number;
  alive: boolean;
  pos: number;
  ...
  if (msg.type === 'match:join' || msg.type === 'room:create') {
    ...
    setTimeout(() => {
      ...
      broadcast(room, 'game:start', {
        seed: room.seed,
        players: room.players.map((p) => ({ id: p.id, nick: p.nick, side: (p as DartsPlayer).side })),
      });
    }, 8000);
  }
    ...
    for (const [, room] of rooms) {
      ...
      if (p) {
        ...
        if (action.alive === false) {
          p.alive = false;
          const alive = room.players.filter((pl) => (pl as DartsPlayer).alive);
          if (alive.length <= 1) {
            room.phase = 'finished';
...
router.get('/health', (_req, res) => res.json({ status: 'ok', game: 'darts', rooms: rooms.size }));
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (raw: RawData) => {
      try {
        ...
        console.error('[darts] 消息处理错误:', err);
      }
    });
    ws.on('close', () => {
      clients.delete(sid);
