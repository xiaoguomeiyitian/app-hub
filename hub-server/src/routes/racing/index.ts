...
export const description = '赛车 - 多人竞速';

export const router = Router();

interface RacingPlayer extends Player {
  side: number;
  score: number;
  alive: boolean;
}
  ...
  if (msg.type === 'match:join' || msg.type === 'room:create') {
    ...
    setTimeout(() => {
      ...
      broadcast(room, 'game:start', {
        seed: room.seed,
        players: room.players.map((p) => ({ id: p.id, nick: p.nick, side: (p as RacingPlayer).side })),
      });
    }, 8000);
  }
    ...
    for (const [, room] of rooms) {
      ...
      if (p) {
        ...
        if (action.alive === false) {
          p.alive = false;
          const alive = room.players.filter((pl) => (pl as RacingPlayer).alive);
          if (alive.length <= 1) {
            room.phase = 'finished';
            broadcast(room, 'game:over', {
              ...
              scores: room.players.map((pl) => ({
                nick: pl.nick,
                score: (pl as RacingPlayer).score,
              })),
            });
          }
        }
...
router.get('/health', (_req, res) => res.json({ status: 'ok', game: 'racing', rooms: rooms.size }));
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (raw: RawData) => {
      try {
        ...
        console.error('[racing] 消息处理错误:', err);
      }
    });
    ws.on('close', () => {
      clients.delete(sid);
