...
export const description = 'Flappy Bird - 竞速飞行';

export const router = Router();

interface FlappyBirdPlayer extends Player {
  side: number;
  score: number;
  alive: boolean;
}
...
function handleMessage(msg: GameMessage, sid: string, ws: WebSocket): void {
  if (msg.type === 'match:join' || msg.type === 'room:create') {
    ...
    setTimeout(() => {
      ...
      broadcast(room, 'game:start', {
        seed: room.seed,
        players: room.players.map((p) => ({ id: p.id, nick: p.nick, side: (p as FlappyBirdPlayer).side })),
      });
    }, 8000);
  }
  ...
  if (msg.type === 'game:action') {
    for (const [, room] of rooms) {
      ...
      if (p) {
        const action = (msg.data as MoveData);
        broadcast(room, 'game:action', { id: sid, ...action });
        if (action.score !== undefined) p.score = action.score as number;
        if (action.pos !== undefined) p.pos = action.pos as number;
        if (action.alive === false) {
          p.alive = false;
          const alive = room.players.filter((pl) => (pl as FlappyBirdPlayer).alive);
          if (alive.length <= 1) {
            room.phase = 'finished';
            broadcast(room, 'game:over', {
              ...
              scores: room.players.map((pl) => ({
                nick: pl.nick,
                score: (pl as FlappyBirdPlayer).score,
              })),
            });
          }
        }
...
router.get('/health', (_req, res) => res.json({ status: 'ok', game: 'flappy-bird', rooms: rooms.size }));
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (raw: RawData) => {
      try {
        ...
        console.error('[flappy-bird] 消息处理错误:', err);
      }
    });
    ws.on('close', () => {
      clients.delete(sid);
